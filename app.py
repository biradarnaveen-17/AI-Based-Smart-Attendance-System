...
# --- Configuration ---
RECOGNITION_THRESHOLD = 0.50 # Changed for higher confidence as requested
PROCESS_EVERY_N_FRAMES = 5   # Process every 5th frame for recognition (can adjust)
VIDEO_UPDATE_INTERVAL_MS = 20 # Update GUI every 20ms (~50 FPS display)
    ...
    def run(self):
        while self.running:
            ...
            if not self.video_capture or not self.video_capture.isOpened():
                self.running = False
                print("VideoProcessor: Video capture not open, stopping.")
                break

            ret, frame = self.video_capture.read()
            if not ret:
                print("VideoProcessor: Failed to grab frame, trying again.")
                time.sleep(0.01) # Small delay to prevent busy-waiting if camera fails
                continue
            ...
            if self.frame_counter % PROCESS_EVERY_N_FRAMES == 0:
                ...
                for i, face_encoding in enumerate(face_encodings):
                    ...
                    if self.known_face_encodings: # Only compare if there are known faces
            ...
            # Always put the latest frame, recognized info might be from a few frames ago
            # If the queue is full, we clear it and put the new one to prioritize freshness.
            try:
                # Clear existing items if any, then put the new one
                while not self.results_queue.empty():
                    self.results_queue.get_nowait()
                self.results_queue.put_nowait({
                    "frame": frame,
                    "recognized_info": recognized_info, # This will be the most recent *processed* info
                    "recognized_count": recognized_count,
                    "is_fresh_data": is_fresh_data
                })
            except queue.Full:
                # This should ideally not happen with the while loop above, but as a safeguard
                pass
...
class AttendanceSystemApp:
    def __init__(self, root):
        ...
        # Queue size adjusted to 2 for slight buffering, but VideoProcessor will prioritize latest
        ...
        self.create_widgets()
        # Initialize _last_displayed_image BEFORE update_video_feed is called
        self._last_displayed_image = None # <--- ADD THIS LINE HERE
        ...
        # self._last_displayed_image = None # If you had it here, move it up as shown
    ...
    def load_encodings(self):
        try:
            ...
            # Update the video processor's known faces directly if it exists
            if hasattr(self, 'video_processor') and self.video_processor.is_alive():
                self.video_processor.known_face_encodings = self.known_face_encodings
                self.video_processor.known_face_names = self.known_face_names
                print("Video processor encodings updated.")
    ...
    def mark_attendance(self, name, confidence):
        # Only mark if not "Unknown" and not already marked today
        if name != "Unknown" and name not in self.students_marked_today:
            ...
            # Insert at the beginning of the treeview for new entries to be visible
            self.attendance_tree.insert("", 0, values=record)
    ...
    def save_attendance_to_db(self, name, date_str, time_str, conf_str):
        ...
        try:
            ...
            # Check for UNIQUE constraint violation, which means already marked
            if "UNIQUE constraint failed: attendance.name, attendance.date, attendance.time" in str(e):
                print(f"Attendance for {name} at {date_str} {time_str} already recorded (DB).")
            else:
                print(f"Error saving attendance to DB for {name}: {e}")
    ...
    def update_video_feed(self):
        # Always try to get the latest frame
        latest_results = None
        while not self.processing_results_queue.empty():
            try:
                latest_results = self.processing_results_queue.get_nowait()
            except queue.Empty:
                break # Queue became empty while we were draining it

        if latest_results:
            frame = latest_results["frame"]
            recognized_info = latest_results["recognized_info"]
            recognized_count = latest_results["recognized_count"]
            is_fresh_data = latest_results["is_fresh_data"]
            ...
            for info in recognized_info:
                ...
                if is_fresh_data: # Only mark attendance if this data came from a fresh recognition frame
                    self.mark_attendance(name, confidence)

            # Update stats only if fresh recognition data was processed
            if is_fresh_data:
            ...
            if target_width > 1 and target_height > 1: # Ensure widget has actual dimensions
            ...
            self._last_displayed_image = img_tk # Store for when queue is empty
        elif self._last_displayed_image:
            # If no new frame, but we have a last displayed image, ensure it's still shown
            # This prevents the video_label from becoming blank
            self.video_label.config(image=self._last_displayed_image)

        self.root.after(VIDEO_UPDATE_INTERVAL_MS, self.update_video_feed) # Schedule next update
    ...
    def open_registration_camera(self, student_name, student_path):
        ...
        if not self.reg_capture.isOpened():
            ...
            self.reg_capture = None
            self.start_main_video_processor() # Attempt to restart main camera
            return
    ...
    def update_reg_feed(self):
        ...
        if not self.reg_capture or not self.reg_capture.isOpened():
             print("Registration capture is not open, stopping reg feed.")
             # No need to call close_registration_camera here directly,
             # as it might cause double-cleanup if called from on_closing or similar
             return
        ...
        if ret:
            ...
            img.thumbnail((640, 480), Image.Resampling.LANCZOS) # Use LANCZOS for quality
            img_tk = ImageTk.PhotoImage(image=img)
            self.reg_video_label.img_tk = img_tk
            self.reg_video_label.config(image=img_tk)
            self.reg_window.after(10, self.update_reg_feed)
        else:
            print("Failed to grab frame from reg_capture, retrying.")
            self.reg_window.after(10, self.update_reg_feed)
    ...
    def start_main_video_processor(self):
        ...
        # Ensure the main video capture is re-initialized if it was released
        if not self.video_capture or not self.video_capture.isOpened():
            self.video_capture = cv2.VideoCapture(0)
            if not self.video_capture.isOpened():
                messagebox.showerror("Camera Error", "Failed to re-open main webcam after registration. Please restart the app.", parent=self.root)
                return
            print("Main video capture re-initialized.")

        # Check if processor is already running or still stopping
        if hasattr(self, 'video_processor') and self.video_processor.is_alive():
             print("Main video processor is already running, no need to restart.")
             return

        # Create and start a new processor instance
    ...
    def _background_encoding_task(self, student_name, student_path):
        ...
        try:
            ...
            # Filter out existing encodings for the student being re-registered
            temp_encodings = []
            temp_names = []
            ...
            for image_name in os.listdir(student_path):
                if image_name.endswith((".jpg", ".png", ".jpeg")):
                    ...
                    except IndexError: # face_recognition.face_encodings might raise IndexError if no faces found
                        print(f"Warning: No face detected in {image_path}. Skipping.")
                    except Exception as e:
    ...
    def manual_mark(self):
        ...
        def submit_manual_mark():
            student_name = name_entry.get().strip()
            if student_name:
                # Always attempt to mark attendance, mark_attendance has checks
                self.mark_attendance(student_name, confidence=None)

                # Force refresh of the treeview to show new manual entry
                self.update_attendance_display(show_popup=False)
    ...
    def on_closing(self):
        ...
        try:
            # Ensure the latest in-memory encodings are saved
            with open('encodings.pkl', 'wb') as f:
        ...
        if hasattr(self, 'video_processor') and self.video_processor.is_alive():
            self.video_processor.stop()
            self.video_processor.join(timeout=2.0) # Give thread a moment to stop
